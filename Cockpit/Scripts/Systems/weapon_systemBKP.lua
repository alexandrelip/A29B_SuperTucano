...
-- =============================================================================
-- Item 11: Modelo balistico por arma. Tabela K_drag baseline (ISA SL) por CLSID.
--
-- K_drag baseline e' o fator efetivo a 1.225 kg/m^3 (ISA SL, sem vento). A
-- correção atmosférica do item 10 modula este valor por densidade real:
--   K_drag_eff = 1 - (1 - K_baseline) * (rho/RHO_ISA_SL)
--
-- Premissa fisica: K_baseline ~ ratio de alcance horizontal real / alcance
-- ballistico ideal (gs * fall_time). Bombas pesadas com baixo Cd*A => K alto;
-- bombas leves ou high-drag (Snakeye/cluster dispensers) => K baixo.
--
-- Valores derivados de:
--  - Tabelas USAF JMEM (Mk-82/83/84 LDGP coeficientes balisticos)
--  - DCS World ballistic params em DB pra cross-reference
--  - Comportamento conhecido do kit Paveway II (planeio terminal pos-bang-bang)
--  - Cluster dispensers abrem cedo => bomba nao chega no alvo, sub-municoes sim
--
-- IMPORTANTE: estes valores sao baselines a refinar empiricamente via log
-- A29B-CCRP (eventos PICKLE/RELEASE). Substituir por valores tunados quando
-- houver dados de campanha (item 12).
...
local LASER_GUIDED_BOMB_K_DRAG = {
    ["{DB769D48-67D7-42ED-A2BE-108D566C8B1E}"] = 0.95,  -- GBU-12 (Mk-82 + Paveway II) - tunado v0.5
    ["{0D33DDAE-524F-4A4E-B5B8-621754FE3ADE}"] = 0.92,  -- GBU-16 (Mk-83 + Paveway II) - tunado v0.5
    ["{GBU_49}"] = 0.95,                                 -- GBU-49 (Mk-82 + Enhanced Paveway II)
}

-- Bombas nao-guiadas (LDGP, retardadas, clusters). Override por CLSID quando
-- comportamento difere significativamente do default 0.88 (que e o baseline
-- empiricamente bom para Mk-82 LDGP conico). Ausencia => fallback DEFAULT.
local UNGUIDED_BOMB_K_DRAG = {
    -- LDGP - General Purpose:
    ["{90321C8E-7ED1-47D4-A160-E074D5ABD902}"] = 0.85,  -- Mk-81 (250 lb) - mais leve, mais arrasto relativo
    ["{BCE4E030-38E9-423E-98ED-24BE3DA87C32}"] = 0.88,  -- Mk-82 LDGP conico (default explicito)
    ["{00F5DAC4-0466-4122-998F-B1A298E34113}"] = 0.90,  -- M-117 (750 lb) - mais pesada, menos arrasto
    -- High-drag retardada (Snakeye com aletas infláveis):
    -- BSU-49 infla ~0.5s pos-release, aumentando Cd ~3-5x. SE DCS modela
    -- o retarder, o K real seria proximo de 0.55 (release ~600m mais cedo
    -- que LDGP). Como nao ha confirmacao empirica de que DCS modela o
    -- retarder via CLSID, usamos valor conservador 0.78 (mesmo de clusters)
    -- ate que o log PICKLE/RELEASE valide. Ajustar para 0.55 se Mk-82SE
    -- consistentemente cair longe (miss longo) em CCRP.
    ["{Mk82SNAKEYE}"] = 0.78,                            -- Mk-82 Snakeye (CONSERVATIVE; needs empirical validation)
    -- Cluster dispensers (abrem cedo, dispersam submunicoes):
    -- Container voa pouco depois do release; alcance horizontal efetivo
    -- e' menor que LDGP equivalente.
    ["{ADD3FAE1-EBF6-4EF9-8EFC-B36B5DDF1E6B}"] = 0.78,  -- Mk-20 Rockeye (cluster Mk-118)
    ["{5335D97A-35A5-4643-9D9B-026C75961E52}"] = 0.78,  -- CBU-97 SFW (BLU-108 sensor-fuzed)
    ["{CBU_105}"] = 0.85,                                -- CBU-105 WCMD (wind-corrected, glide)
}

local DEFAULT_BOMB_K_DRAG = 0.88

-- =============================================================================
-- Recalibracao CCRP (Item 6) e gates de envelope de release (Item 7).
--
-- CCRP_LEAD_TIME: antecipa o trigger de release para compensar latencia
-- sistematica (input -> ejector). Gate dispara quando CCRP_TIME <= LEAD_TIME.
-- Default 0.0 mantem o comportamento atual; ajuste empirico recomendado a
-- partir do log CCRP (DCS.log: "A29B-CCRP").
--
-- RELEASE_ENVELOPE_*: limites operacionais do envelope de release. Aplicados
-- no momento do pickle em modos automaticos (CCIP/CCRP). Modos MAN e GUN
-- bypassam (override manual / strafe). Sao limites conservadores baseados
-- em pratica CAS para evitar releases instaveis ou abaixo do minimum AGL.
local CCRP_LEAD_TIME_DEFAULT = 0.0
local RELEASE_ENVELOPE_BANK_MAX_RAD = math.rad(75)   -- |roll| > 75 deg => unsafe
local RELEASE_ENVELOPE_PITCH_MAX_UP_RAD = math.rad(30)   -- pitch > +30 deg unsafe
local RELEASE_ENVELOPE_PITCH_MAX_DOWN_RAD = math.rad(60) -- pitch < -60 deg unsafe
local RELEASE_ENVELOPE_MIN_AGL_M = 60.0              -- ~200 ft AGL bomb arming floor

local ccrp_pickle_snapshot = nil  -- preenchido no pickle press, consumido no release real

-- Verifica se o envelope esta seguro para release automatico.
-- Retorna (ok:bool, reason:string|nil).
local function is_release_envelope_safe(master_mode_arg)
    -- Bypass para modos manuais/gun (override do piloto, strafe rasante).
    if master_mode_arg == AVIONICS_MASTER_MODE_ID.MAN
       or master_mode_arg == AVIONICS_MASTER_MODE_ID.GUN
       or master_mode_arg == AVIONICS_MASTER_MODE_ID.GUN_R then
        return true, nil
    end

    local roll = sensor_data:getRoll()
    if math.abs(roll) > RELEASE_ENVELOPE_BANK_MAX_RAD then
        return false, string.format("BANK %.0f deg > %.0f", math.deg(math.abs(roll)), math.deg(RELEASE_ENVELOPE_BANK_MAX_RAD))
    end

    local pitch = sensor_data:getPitch()
    if pitch > RELEASE_ENVELOPE_PITCH_MAX_UP_RAD then
        return false, string.format("PITCH UP %.0f deg > %.0f", math.deg(pitch), math.deg(RELEASE_ENVELOPE_PITCH_MAX_UP_RAD))
    end
    if pitch < -RELEASE_ENVELOPE_PITCH_MAX_DOWN_RAD then
        return false, string.format("PITCH DOWN %.0f deg < -%.0f", math.deg(pitch), math.deg(RELEASE_ENVELOPE_PITCH_MAX_DOWN_RAD))
    end

    local ralt = sensor_data.getRadarAltitude()
    if ralt ~= nil and ralt < RELEASE_ENVELOPE_MIN_AGL_M then
        return false, string.format("AGL %.0f m < %.0f", ralt, RELEASE_ENVELOPE_MIN_AGL_M)
    end

    return true, nil
end

-- Loga linha CCRP no DCS.log (categoria "A29B-CCRP") para coleta empirica.
-- Formato grepavel para extracao posterior em CSV via PowerShell/grep.
local function log_ccrp_event(event, fields)
    if log == nil or log.write == nil then return end
    local parts = { "event=" .. tostring(event) }
    if fields then
        for k, v in pairs(fields) do
            parts[#parts + 1] = tostring(k) .. "=" .. tostring(v)
        end
    end
    log.write("A29B-CCRP", log.INFO, table.concat(parts, " "))
end
...
local function get_weapon_k_drag(station_index)
    if station_index == nil or station_index <= 0 then
        return DEFAULT_BOMB_K_DRAG
    end
    local station_info = dev:get_station_info(station_index - 1)
    if station_info ~= nil then
        local clsid = station_info["CLSID"]
        -- Item 11: consulta tabela guiada primeiro (Paveway II tunadas), depois
        -- a tabela de bombas nao-guiadas, e por fim cai no default 0.88.
        local k = LASER_GUIDED_BOMB_K_DRAG[clsid]
        if k ~= nil then
            return k
        end
        k = UNGUIDED_BOMB_K_DRAG[clsid]
        if k ~= nil then
            return k
        end
    end
...
local function should_hold_flir_designation()
    return auto_lase_timeout > 0 and wpn_ag_sel > 0 and is_laser_guided_bomb_station(wpn_ag_sel)
end
...
local function update_ag_sel_next(byname)
    ...
    if pos > 0 then
        ...
        if  wpn_sto_type[wpn_ag_sel] == WPN_WEAPON_TYPE_IDS.AG_UNGUIDED_ROCKET and (master_mode == AVIONICS_MASTER_MODE_ID.DTOS or master_mode == AVIONICS_MASTER_MODE_ID.DTOS_R or master_mode == AVIONICS_MASTER_MODE_ID.CCRP or master_mode == AVIONICS_MASTER_MODE_ID.CCRP_R) then
            if master_mode == AVIONICS_MASTER_MODE_ID.DTOS_R or master_mode == AVIONICS_MASTER_MODE_ID.CCRP_R then 
...
local function update_master_mode_changed()
    ...
    elseif master_mode == AVIONICS_MASTER_MODE_ID.CCRP and WPN_RALT:get() == 1 then master_mode = AVIONICS_MASTER_MODE_ID.CCRP_R
    elseif master_mode == AVIONICS_MASTER_MODE_ID.CCRP_R and WPN_RALT:get() == 0 then master_mode = AVIONICS_MASTER_MODE_ID.CCRP
    end

    if (master_mode >= AVIONICS_MASTER_MODE_ID.GUN and master_mode <= AVIONICS_MASTER_MODE_ID.MAN) or master_mode == AVIONICS_MASTER_MODE_ID.CCRP_R then
...
-- Item 9: Regra hibrida BARO/RALT por envelope de voo/terreno.
-- Constantes de selecao de fonte de altitude para CCIP/CCRP/CCIP delayed.
-- Premissa fisica: o raltimetro mede slant-range para o terreno (perpendicular ao
-- eixo do corpo). A projecao vertical AGL = Ralt * cos(pitch) * cos(roll). Em
-- atitudes extremas (>60 deg) o erro geometrico e' inaceitavel; alem do alcance
-- operacional do raltimetro (~1500 m), o sensor satura. Nestas condicoes a
-- formula hibrida (cache RALT + delta BARO) toma conta como fallback suave.
local RALT_MAX_VALID_M           = 1500.0       -- limite operacional efetivo do raltimetro
local RALT_REENGAGE_M            = 1450.0       -- histerese: so reengaja envelope se < REENGAGE
local RALT_MIN_VALID_M           = 0.5          -- evita zero/negativo (sensor glitch)
local RALT_MAX_JUMP_PER_FRAME_M  = 50.0         -- rejeita salto > 50 m em um frame (penhasco/glitch)
local RALT_ATTITUDE_LIMIT_RAD    = math.rad(60) -- alem disso a projecao cos*cos perde acuracia
local RALT_CACHE_STALE_S         = 60.0         -- cache > 60 s sem atualizacao = stale

local Ralt_last        = 1600.0
local Balt_last        = 1600.0
local Ralt_cache_time  = -1.0    -- model time da ultima atualizacao valida; -1 = nunca atualizado
local Ralt_in_envelope = false   -- estado: dentro do envelope util (com histerese)

-- Atualiza o cache de altitude RALT/BARO usado por CCIP, CCIP delayed e CCRP_R.
-- Chamado uma vez por frame no topo de update_ag() para garantir cadencia uniforme
-- da solucao CCIP/CCRP, independente do master mode ativo.
--
-- Regras de aceitacao do RALT bruto:
--   1) Ralt nao-nil e >= RALT_MIN_VALID_M;
--   2) Ralt < limite com histerese (REENGAGE em re-entrada, MAX_VALID em saida);
--   3) |pitch| < 60 deg e |roll| < 60 deg (envelope de projecao geometrica);
--   4) salto frame-a-frame <= RALT_MAX_JUMP_PER_FRAME_M (rejeita glitch/penhasco).
-- Quando aceito, atualiza Ralt_last/Balt_last/Ralt_cache_time e marca envelope ON.
-- Caso contrario, NAO apaga o cache: o helper get_terrain_height_agl decidira o
-- modo de fallback (BARO delta ou cache stale).
local function update_ralt_baro_cache()
    local Ralt = sensor_data.getRadarAltitude()
    local Balt = sensor_data.getBarometricAltitude()
    local p_pitch_abs = math.abs(sensor_data:getPitch())
    local p_roll_abs  = math.abs(sensor_data:getRoll())
    local now = get_absolute_model_time()

    local attitude_ok = (p_pitch_abs < RALT_ATTITUDE_LIMIT_RAD) and (p_roll_abs < RALT_ATTITUDE_LIMIT_RAD)
    local ralt_finite = (Ralt ~= nil) and (Ralt >= RALT_MIN_VALID_M)

    -- Histerese para evitar flicker no limite operacional do raltimetro.
    local upper_limit = Ralt_in_envelope and RALT_MAX_VALID_M or RALT_REENGAGE_M
    local within_range = ralt_finite and (Ralt < upper_limit)

    -- Rejeita salto grande (sensor glitch ou cliff) somente se ja estavamos no envelope.
    local jump_ok = true
    if Ralt_in_envelope and ralt_finite then
        if math.abs(Ralt - Ralt_last) > RALT_MAX_JUMP_PER_FRAME_M then
            jump_ok = false
        end
    end

    if within_range and attitude_ok and jump_ok and Balt ~= nil then
        Ralt_last = Ralt
        Balt_last = Balt
        Ralt_cache_time = now
        Ralt_in_envelope = true
    elseif not within_range then
        Ralt_in_envelope = false
        -- cache preservado para fallback BARO-delta
    end
end

-- Helper para obter altura AGL projetada verticalmente (m), com selecao
-- contextual de fonte conforme envelope/staleness:
--   1) RALT cru se em envelope e leitura valida (mais preciso e responsivo);
--   2) RALT_last + (BARO atual - BARO_last) se cache fresco (formula hibrida classica);
--   3) Ralt_last cru se cache stale (ultima alternativa antes de o caller usar NAV alt).
-- Sempre retorna (h_agl_m: number, source: string). h_agl >= 0.
local function get_terrain_height_agl(p_pitch, p_roll)
    local cos_factor = math.cos(math.abs(p_pitch)) * math.cos(math.abs(p_roll))
    local now = get_absolute_model_time()

    if Ralt_in_envelope then
        local Ralt = sensor_data.getRadarAltitude()
        if Ralt ~= nil and Ralt >= RALT_MIN_VALID_M and Ralt < RALT_MAX_VALID_M then
            return Ralt * cos_factor, "RALT"
        end
    end

    local Balt = sensor_data.getBarometricAltitude()
    local cache_age = (Ralt_cache_time > 0) and (now - Ralt_cache_time) or math.huge
    if cache_age < RALT_CACHE_STALE_S and Balt ~= nil then
        local h = Ralt_last + (Balt - Balt_last)
        if h < 0 then h = 0 end
        return h * cos_factor, "RALT+BARO"
    end

    -- Fallback duro: cache stale, mantem ultima leitura conhecida.
    return Ralt_last * cos_factor, "STALE"
end

-- =============================================================================
-- Item 10: Correções atmosféricas para CCRP (densidade + temperatura + vento).
--
-- Modelo: ISA padrão para troposfera (h <= 11 km), com correção de temperatura
-- (e portanto densidade) derivada de Mach + TAS quando disponíveis. Vento é
-- inferido de (V_ground - V_air_estimado) via heading + pitch + TAS, ignorando
-- AOA/AOS (erro tipicamente < 10% para voo cruzeiro CAS).
--
-- Aplicado em CCRP (CCRP/CCRP_R): densidade modula K_drag (mais ar = mais
-- arrasto = bomba viaja menos); vento along-track entra na velocidade média
-- da bomba durante o fall_time. Modelo combinado:
--   K_drag_eff = 1 - (1 - K_drag_baseline) * (rho_actual / RHO_ISA_SL)
--   travel    = (gs * K_drag_eff + wind_along * (1 - K_drag_eff)) * fall_time
--
-- Limites operacionais (clamp K_drag em [0.5, 0.99]) impedem extrapolação além
-- do envelope físico realista. CCIP non-_R via Calculate() nativo nao e
-- afetado (DCS faz própria balística). CCIP delayed e geometrico, sem K_drag.
local ISA_T0_K          = 288.15           -- ISA SL temperature (K)
local ISA_LAPSE_RATE    = 0.0065           -- K/m (0-11 km troposphere)
local ISA_P0_PA         = 101325.0         -- ISA SL pressure (Pa)
local ISA_R_AIR         = 287.05           -- specific gas constant (J/(kg*K))
local ISA_RHO0          = 1.225            -- ISA SL density (kg/m^3)
local SOUND_SPEED_K     = 20.0463          -- speed of sound: a = K * sqrt(T_K)
local ISA_TROPOPAUSE_M  = 11000.0          -- limite linear lapse rate
local TAS_VALID_MIN_MS  = 15.0             -- TAS minimo p/ reconstruir vento
local K_DRAG_MIN        = 0.50             -- clamp inferior (sanity)
local K_DRAG_MAX        = 0.99             -- clamp superior (vacuo aproximado)

-- Pressao atmosferica ISA (Pa) na altitude h (m). Geopotencial ~ geometrica < 11 km.
local function isa_pressure_at(h)
    local h_eff = math.max(0.0, math.min(h or 0.0, ISA_TROPOPAUSE_M))
    local ratio = 1.0 - (ISA_LAPSE_RATE * h_eff) / ISA_T0_K
    return ISA_P0_PA * (ratio ^ 5.2561)
end

-- Temperatura ISA (K) na altitude h.
local function isa_temperature_at(h)
    local h_eff = math.max(0.0, math.min(h or 0.0, ISA_TROPOPAUSE_M))
    return ISA_T0_K - ISA_LAPSE_RATE * h_eff
end

-- Densidade real do ar (kg/m^3). Usa Mach+TAS para corrigir temperatura quando
-- disponivel; senao cai pra ISA. Retorna (rho, T_K, source) onde
-- source ∈ {"MACH", "ISA"}.
local function get_air_density_kg_m3()
    local h = sensor_data.getBarometricAltitude() or 0.0
    local p = isa_pressure_at(h)
    local mach = sensor_data.getMachNumber and sensor_data.getMachNumber() or nil
    local tas  = sensor_data.getTrueAirSpeed and sensor_data.getTrueAirSpeed() or nil
    if mach ~= nil and tas ~= nil and mach > 0.05 and tas > 5.0 then
        local a = tas / mach
        local T_K = (a / SOUND_SPEED_K) ^ 2
        if T_K > 200.0 and T_K < 350.0 then
            return p / (ISA_R_AIR * T_K), T_K, "MACH"
        end
    end
    local T_isa = isa_temperature_at(h)
    return p / (ISA_R_AIR * T_isa), T_isa, "ISA"
end

-- Vetor de vento no frame do mundo (wx, wy, wz em m/s) inferido de
-- V_ground - V_air_estimado. Usa convencao p_hdg = 2*pi - heading (coerente com
-- hud.lua: v_hdg = atan2(v_z, v_x) = p_hdg quando nao ha vento). Negligencia
-- AOA/AOS. Se TAS < TAS_VALID_MIN_MS, retorna (0, 0, 0).
local function get_wind_world()
    local tas = sensor_data.getTrueAirSpeed and sensor_data.getTrueAirSpeed() or nil
    local vx, vy, vz = sensor_data.getSelfVelocity()
    if tas == nil or tas < TAS_VALID_MIN_MS or vx == nil then
        return 0.0, 0.0, 0.0
    end
    local p_hdg = 2.0 * math.pi - sensor_data:getHeading()
    local p_pitch = sensor_data:getPitch()
    local cos_p = math.cos(p_pitch)
    local ax = tas * cos_p * math.cos(p_hdg)
    local ay = tas * math.sin(p_pitch)
    local az_w = tas * cos_p * math.sin(p_hdg)
    return vx - ax, vy - ay, vz - az_w
end

-- Componente do vento (m/s) projetada no ground track horizontal.
-- Sinal positivo = vento de cauda (acelera bomba); negativo = vento de proa.
-- Se ground speed for residual (gs^2 < 1.0 m^2/s^2), retorna 0.
local function get_wind_along_ground_track()
    local vx, _, vz = sensor_data.getSelfVelocity()
    local gs2 = (vx or 0.0) * (vx or 0.0) + (vz or 0.0) * (vz or 0.0)
    if gs2 < 1.0 then return 0.0 end
    local gs = math.sqrt(gs2)
    local wx, _, wz = get_wind_world()
    return (wx * vx + wz * vz) / gs
end
...
local function  update_ccrp()
    if (master_mode == AVIONICS_MASTER_MODE_ID.CCRP or master_mode == AVIONICS_MASTER_MODE_ID.CCRP_R) and wpn_ag_sel > 0 and wpn_sto_count[wpn_ag_sel]>0  then
        wpn_target = {}
        ...
        -- CCRP_R: substitui altitude do alvo pela elevação do terreno via raltímetro.
        -- Item 9: usa helper hibrido (RALT cru / RALT+BARO / cache stale) com envelope.
        if master_mode == AVIONICS_MASTER_MODE_ID.CCRP_R then
            local _, y_ac, _ = sensor_data.getSelfCoordinates()
            local p_pitch = sensor_data:getPitch()
            local p_roll  = sensor_data:getRoll()
            local h_agl, _alt_src = get_terrain_height_agl(p_pitch, p_roll)
            wpn_target.alt_m = y_ac - h_agl
        end
        ...
        --
        -- Item 10: correção atmosférica completa.
        --   Densidade: K_drag_eff = 1 - (1 - K_drag_baseline) * (rho/RHO_ISA_SL).
        --     Em altitude ou dia quente ρ < ρ0 ⇒ menos arrasto ⇒ K_drag sobe ⇒ bomba viaja mais.
        --     Em SL ou dia frio ρ > ρ0 ⇒ mais arrasto ⇒ K_drag desce ⇒ bomba viaja menos.
        --   Vento along-track: travel = (gs*K_drag + w_along*(1-K_drag)) * fall_time.
        --     Vento de cauda (w>0) estende; vento de proa (w<0) encurta.
        --   Quando ρ→0 (vácuo) K_drag→1 e travel→gs*fall_time (puro balístico).
        local K_drag_baseline = get_weapon_k_drag(wpn_ag_sel)
        local rho_air, _T_K, _rho_src = get_air_density_kg_m3()
        local rho_ratio = rho_air / ISA_RHO0
        local K_drag = 1.0 - (1.0 - K_drag_baseline) * rho_ratio
        if K_drag < K_DRAG_MIN then K_drag = K_DRAG_MIN end
        if K_drag > K_DRAG_MAX then K_drag = K_DRAG_MAX end
        local wind_along = get_wind_along_ground_track()
        local travel_dist = (gs * K_drag + wind_along * (1.0 - K_drag)) * fall_time
...
local function  update_ccip()
    ...
    if ((master_mode == AVIONICS_MASTER_MODE_ID.CCIP or master_mode == AVIONICS_MASTER_MODE_ID.CCIP_R) and wpn_ag_sel > 0 and wpn_sto_count[wpn_ag_sel]>0) or (master_mode == AVIONICS_MASTER_MODE_ID.GUN or master_mode == AVIONICS_MASTER_MODE_ID.GUN_R)  then
        ...
        -- Item 9: altura AGL via helper hibrido (RALT/BARO/STALE) com envelope.
        -- Cache atualizado por update_ralt_baro_cache() no topo de update_ag().
        local h_agl, _alt_src = get_terrain_height_agl(p_pitch, p_roll)
        h0 = y - h_agl
...
function update()
    ...
    if wpn_ripple_count > 0 then
        local ccrp_lead = CCRP_LEAD_TIME_DEFAULT
        if ((master_mode == AVIONICS_MASTER_MODE_ID.CCRP or master_mode == AVIONICS_MASTER_MODE_ID.CCRP_R) and WPN.CCRP_TIME:get() > ccrp_lead) then
        elseif ((master_mode == AVIONICS_MASTER_MODE_ID.CCIP or master_mode == AVIONICS_MASTER_MODE_ID.CCIP_R) and WPN.CCIP_DELAYED:get() == 1 and WPN.CCIP_DELAYED_TIME:get() > ccrp_lead) then
        else
            ...
            while wpn_ripple_elapsed >= wpn_ripple_interval and wpn_ripple_count > 0 do
                ...
                if (master_mode == AVIONICS_MASTER_MODE_ID.CCIP or master_mode == AVIONICS_MASTER_MODE_ID.CCIP_R or master_mode == AVIONICS_MASTER_MODE_ID.CCRP or master_mode == AVIONICS_MASTER_MODE_ID.CCRP_R) and WPN_SELECTED_WEAPON_TYPE:get() == WPN_WEAPON_TYPE_IDS.AG_UNGUIDED_BOMB then
                ...
                if released_this_step then
                    -- Log do release real para recalibracao CCRP (Item 6 + Item 11).
                    -- Captura erro temporal: t_press -> t_release vs ccrp_time_pred.
                    -- Inclui K_baseline + K_eff + atmosfera p/ regressao de K_drag.
                    if ccrp_pickle_snapshot ~= nil then
                        local t_now = get_absolute_model_time()
                        log_ccrp_event("RELEASE", {
                            mode = master_mode,
                            weapon = ccrp_pickle_snapshot.weapon,
                            t_elapsed = t_now - ccrp_pickle_snapshot.t_press,
                            ccrp_time_pred = ccrp_pickle_snapshot.ccrp_time_pred,
                            fly_time_pred = ccrp_pickle_snapshot.fly_time_pred,
                            ccrp_time_now = WPN.CCRP_TIME:get(),
                            gs = ccrp_pickle_snapshot.gs,
                            ralt = ccrp_pickle_snapshot.ralt,
                            lead_time = ccrp_pickle_snapshot.lead_time,
                            k_baseline = ccrp_pickle_snapshot.k_baseline,
                            k_eff = ccrp_pickle_snapshot.k_eff,
                            rho_kg_m3 = ccrp_pickle_snapshot.rho_kg_m3,
                            T_K = ccrp_pickle_snapshot.T_K,
                            wind_along_ms = ccrp_pickle_snapshot.wind_along_ms,
                        })
                        if wpn_ripple_count == 0 then
                            ccrp_pickle_snapshot = nil
                        end
                    end
...
function SetCommand(command,value)
    ...
    elseif command == Keys.WeaponRelease then
        ...
        if get_wpn_ag_ready() and not get_avionics_master_mode_ag_gun() and value == 1 then
            -- Gate de envelope (Item 7): bloqueia pickle fora dos limites operacionais
            -- (bank, pitch, AGL minimo). Modos MAN/GUN bypassam (override do piloto).
            local envelope_ok, envelope_reason = is_release_envelope_safe(master_mode)
            if not envelope_ok then
                log_ccrp_event("PICKLE_BLOCKED", {
                    mode = master_mode,
                    reason = envelope_reason,
                    weapon = (wpn_ag_sel > 0 and wpn_sto_name[wpn_ag_sel]) or "n/a",
                })
                return
            end
            ...
            if master_mode == AVIONICS_MASTER_MODE_ID.CCRP or master_mode == AVIONICS_MASTER_MODE_ID.CCRP_R then
                wpn_ripple_count = wpn_ripple_count + 1
                -- Snapshot CCRP no pickle press (Item 6 + Item 10 + Item 11): coleta
                -- empirica para recalibracao do trigger e do K_drag baseline. Inclui
                -- contexto atmosferico (rho/T/wind) e ballistico (K_baseline +
                -- K_drag_eff pos-densidade) para fechar o ciclo PICKLE -> RELEASE
                -- com dados suficientes para regredir K_baseline por arma.
                local rho_p, T_K_p, T_src_p = get_air_density_kg_m3()
                local k_baseline_p = get_weapon_k_drag(wpn_ag_sel)
                local k_eff_p = 1.0 - (1.0 - k_baseline_p) * (rho_p / ISA_RHO0)
                if k_eff_p < K_DRAG_MIN then k_eff_p = K_DRAG_MIN end
                if k_eff_p > K_DRAG_MAX then k_eff_p = K_DRAG_MAX end
                ccrp_pickle_snapshot = {
                    t_press = get_absolute_model_time(),
                    ccrp_time_pred = WPN.CCRP_TIME:get(),
                    fly_time_pred = WPN.TIME_TO_IMPACT:get(),
                    gs = get_avionics_gs() or 0,
                    ralt = sensor_data.getRadarAltitude() or 0,
                    pitch_deg = math.deg(sensor_data:getPitch()),
                    roll_deg = math.deg(sensor_data:getRoll()),
                    weapon = (wpn_ag_sel > 0 and wpn_sto_name[wpn_ag_sel]) or "n/a",
                    mode = master_mode,
                    lead_time = CCRP_LEAD_TIME_DEFAULT,
                    rho_kg_m3 = rho_p,
                    T_K = T_K_p,
                    T_src = T_src_p,
                    wind_along_ms = get_wind_along_ground_track(),
                    rho_ratio = rho_p / ISA_RHO0,
                    k_baseline = k_baseline_p,
                    k_eff = k_eff_p,
                }
                log_ccrp_event("PICKLE", ccrp_pickle_snapshot)
